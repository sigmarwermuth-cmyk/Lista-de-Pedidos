...
import { X, Plus, Search, Edit2, Trash2, RotateCcw, Package, Check, AlertCircle, Barcode } from 'lucide-react';
...
export const ProductManagerModal: React.FC<ProductManagerModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialEditingProduct) {
        ...
        setFormBarcode(initialEditingProduct.barcode || '');
  ...
  }, [isOpen, initialEditingProduct, initialBarcode]);
                    ...
                    {filteredProducts.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="py-2 px-3 font-mono text-slate-600 text-xs">
                          {p.barcode ? (
                            <span className="px-1.5 py-0.5 bg-slate-100 border border-slate-200 rounded font-semibold text-[11px] text-slate-800">
                              {p.barcode}
                            </span>
                          ) : (
            ...
            <form onSubmit={handleSaveForm} className="space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-emerald-600 font-medium">Bipe com o leitor</span>
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-xl text-xs sm:text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 focus:bg-white"
                  />
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  💡 No desktop, com o cursor neste campo, basta apertar o gatilho do leitor.
                </p>
              </div>
